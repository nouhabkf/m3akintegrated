...
export class TransportReviewService {
  ...
  async create(transportId: string, userId: string, dto: CreateTransportReviewDto) {
    ...
    if (transport.statut !== TransportStatut.TERMINEE) {
      throw new ForbiddenException('Vous ne pouvez évaluer que des transports terminés');
    }
    ...
    const existing = await this.transportReviewModel
      ...
      .exec();
    if (existing) {
      throw new ConflictException('Une évaluation existe déjà pour ce transport');
    }
